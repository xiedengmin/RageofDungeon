...
namespace Godot.Mathematics
{
    public static partial class noise
    {
        ...
        public static Vector2 cellular2x2x2(Vector3 P)
        {
            ...
            Vector3 Pf = frac(P);
            Vector4 Pfx = Pf.x + Vector4(0.0f, -1.0f, 0.0f, -1.0f);
            Vector4 Pfy = Pf.y + Vector4(0.0f, 0.0f, -1.0f, -1.0f);
            Vector4 p = permute(Pi.x + Vector4(0.0f, 1.0f, 0.0f, 1.0f));
            p = permute(p + Pi.y + Vector4(0.0f, 0.0f, 1.0f, 1.0f));
            Vector4 p1 = permute(p + Pi.z); // z+0
            Vector4 p2 = permute(p + Pi.z + Vector4(1.0f,1.0f,1.0f,1.0f)); // z+1
            Vector4 ox1 = frac(p1 * K) - Ko;
            Vector4 oy1 = mod7(floor(p1 * K)) * K - Ko;
            Vector4 oz1 = floor(p1 * K2) * Kz - Kzo; // p1 < 289 guaranteed
            Vector4 ox2 = frac(p2 * K) - Ko;
            Vector4 oy2 = mod7(floor(p2 * K)) * K - Ko;
            Vector4 oz2 = floor(p2 * K2) * Kz - Kzo;
            Vector4 dx1 = Pfx + jitter * ox1;
            Vector4 dy1 = Pfy + jitter * oy1;
            Vector4 dz1 = Pf.z + jitter * oz1;
            Vector4 dx2 = Pfx + jitter * ox2;
            Vector4 dy2 = Pfy + jitter * oy2;
            Vector4 dz2 = Pf.z - 1.0f + jitter * oz2;
            Vector4 d1 = dx1 * dx1 + dy1 * dy1 + dz1 * dz1; // z+0
            Vector4 d2 = dx2 * dx2 + dy2 * dy2 + dz2 * dz2; // z+1

            // Sort out the two smallest distances (F1, F2)

            // Do it right and sort out both F1 and F2
            Vector4 d = min(d1,d2); // F1 is now in d
