...
namespace Godot.Mathematics
{
    public static partial class noise
    {
        ...
        public static Vector2 cellular(Vector3 P)
        {
            ...
            Vector3 Pi = mod289(floor(P));
            Vector3 Pf = frac(P) - 0.5f;

            Vector3 Pfx = Pf.x + Vector3(1.0f, 0.0f, -1.0f);
            Vector3 Pfy = Pf.y + Vector3(1.0f, 0.0f, -1.0f);
            Vector3 Pfz = Pf.z + Vector3(1.0f, 0.0f, -1.0f);

            Vector3 p = permute(Pi.x + Vector3(-1.0f, 0.0f, 1.0f));
            Vector3 p1 = permute(p + Pi.y - 1.0f);
            Vector3 p2 = permute(p + Pi.y);
            Vector3 p3 = permute(p + Pi.y + 1.0f);

            Vector3 p11 = permute(p1 + Pi.z - 1.0f);
            Vector3 p12 = permute(p1 + Pi.z);
            Vector3 p13 = permute(p1 + Pi.z + 1.0f);

            Vector3 p21 = permute(p2 + Pi.z - 1.0f);
            Vector3 p22 = permute(p2 + Pi.z);
            Vector3 p23 = permute(p2 + Pi.z + 1.0f);

            Vector3 p31 = permute(p3 + Pi.z - 1.0f);
            Vector3 p32 = permute(p3 + Pi.z);
            Vector3 p33 = permute(p3 + Pi.z + 1.0f);

            Vector3 ox11 = frac(p11 * K) - Ko;
            Vector3 oy11 = mod7(floor(p11 * K)) * K - Ko;
            Vector3 oz11 = floor(p11 * K2) * Kz - Kzo; // p11 < 289 guaranteed

            Vector3 ox12 = frac(p12 * K) - Ko;
            Vector3 oy12 = mod7(floor(p12 * K)) * K - Ko;
            Vector3 oz12 = floor(p12 * K2) * Kz - Kzo;

            Vector3 ox13 = frac(p13 * K) - Ko;
            Vector3 oy13 = mod7(floor(p13 * K)) * K - Ko;
            Vector3 oz13 = floor(p13 * K2) * Kz - Kzo;

            Vector3 ox21 = frac(p21 * K) - Ko;
            Vector3 oy21 = mod7(floor(p21 * K)) * K - Ko;
            Vector3 oz21 = floor(p21 * K2) * Kz - Kzo;

            Vector3 ox22 = frac(p22 * K) - Ko;
            Vector3 oy22 = mod7(floor(p22 * K)) * K - Ko;
            Vector3 oz22 = floor(p22 * K2) * Kz - Kzo;

            Vector3 ox23 = frac(p23 * K) - Ko;
            Vector3 oy23 = mod7(floor(p23 * K)) * K - Ko;
            Vector3 oz23 = floor(p23 * K2) * Kz - Kzo;

            Vector3 ox31 = frac(p31 * K) - Ko;
            Vector3 oy31 = mod7(floor(p31 * K)) * K - Ko;
            Vector3 oz31 = floor(p31 * K2) * Kz - Kzo;

            Vector3 ox32 = frac(p32 * K) - Ko;
            Vector3 oy32 = mod7(floor(p32 * K)) * K - Ko;
            Vector3 oz32 = floor(p32 * K2) * Kz - Kzo;

            Vector3 ox33 = frac(p33 * K) - Ko;
            Vector3 oy33 = mod7(floor(p33 * K)) * K - Ko;
            ...
            Vector3 d11 = dx11 * dx11 + dy11 * dy11 + dz11 * dz11;
            Vector3 d12 = dx12 * dx12 + dy12 * dy12 + dz12 * dz12;
            Vector3 d13 = dx13 * dx13 + dy13 * dy13 + dz13 * dz13;
            Vector3 d21 = dx21 * dx21 + dy21 * dy21 + dz21 * dz21;
            Vector3 d22 = dx22 * dx22 + dy22 * dy22 + dz22 * dz22;
            Vector3 d23 = dx23 * dx23 + dy23 * dy23 + dz23 * dz23;
            Vector3 d31 = dx31 * dx31 + dy31 * dy31 + dz31 * dz31;
            Vector3 d32 = dx32 * dx32 + dy32 * dy32 + dz32 * dz32;
            Vector3 d33 = dx33 * dx33 + dy33 * dy33 + dz33 * dz33;
