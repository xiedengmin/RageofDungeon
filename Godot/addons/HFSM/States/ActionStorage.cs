...
public class ActionStorage<TEvent> {
    ...
    private TTarget TryGetAndCastAction<TTarget>(TEvent trigger) where TTarget : Delegate {
        _actionsByEvent.TryGetValue(trigger, out Delegate action);
        return action is null ? null : (TTarget)action ?? throw new InvalidOperationException(ExceptionFormatter.Format(
